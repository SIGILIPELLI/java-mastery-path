...
Test containers (Testcontainers) work by talking to the local Docker
daemon's API to spin up a **real, ephemeral container** (Postgres,
Kafka, etc.) before your test class runs, exposing its randomly-assigned
host port back to the JVM test process through the Docker API's port
mapping — your JDBC URL/connection string is rewritten at runtime to
point at that mapped port, so the test genuinely talks to real database
wire-protocol behavior, not a mock, and the container is torn down via a
JVM shutdown hook the library registers automatically.

Mockito's `mock()` builds actual **subclasses at runtime** via
ByteBuddy (bytecode generation), intercepting every method call through
a generated proxy that consults a `MockHandler` — when you
`verify(mock).someMethod()`, Mockito is inspecting an internal
invocation log the proxy recorded, not doing anything reflective at
verification time; the interception happens once, at call time, via
real generated bytecode, not dynamic reflection per call.

Test parallelization (JUnit 5's parallel execution) runs test classes
on a `ForkJoinPool`-backed thread pool; because JUnit 5 also creates a
new test-instance-per-method by default, parallel test methods within a
class don't share instance state — the actual concurrency hazard is
almost always **shared static state or a shared external resource**
(a test database, a static cache) that multiple parallel tests mutate
without isolation.
